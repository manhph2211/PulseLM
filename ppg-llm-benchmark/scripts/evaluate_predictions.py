...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to eval_predictions.jsonl produced by scripts/test.py.",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        required=True,
        help="Directory used to save the aggregated metrics JSON.",
    )
    ...
    metrics = {
        ...
        "pred_in_options_rate": safe_div(
            overall["pred_in_options"], overall["num_predictions"]
        ),
        "strict_answer_tag_rate": safe_div(
            overall["strict_answer_tag"], overall["num_predictions"]
        ),
    ...
    for question_category, counter in sorted(per_question.items()):
        ...
        metrics["per_question_category"][question_category] = {
            ...
            "pred_in_options_rate": safe_div(
                counter["pred_in_options"], num_predictions
            ),
            "strict_answer_tag_rate": safe_div(
                counter["strict_answer_tag"], num_predictions
            ),
        }
