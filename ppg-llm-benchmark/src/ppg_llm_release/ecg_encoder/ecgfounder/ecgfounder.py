...
def get_ecgfounder_model(checkpoint_path='checkpoints/ecgfounder/12_lead_ECGFounder.pth'):
    lead_n = int(checkpoint_path.split('/')[-1].split('_')[0])
    model = Net1D(
        in_channels=lead_n, 
        base_filters=64, 
        ratio=1, 
...
def get_ecgfounder_model_1(checkpoint_path='/node1/jinyang_node1/project_node1/ppg_project/checkpoints/ecgfounder/1_lead_ECGFounder.pth'):
    lead_n = int(checkpoint_path.split('/')[-1].split('_')[0])
    model = Net1D(
        in_channels=lead_n, 
        base_filters=64, 
...
def get_ecgfounder_ecg_feats(model, ecgs):
    try:
        ecg_feats = model(ecgs)
    except:
        ecg_feats = model(ecgs[:,0:1,:])  # 1 lead only
    return ecg_feats
...
if __name__ == "__main__":
    ...
    x = torch.randn(2, lead_n, 5000) # 10s * 500 Hz
    ckpt = torch.load(f'checkpoints/ecgfounder/{lead_n}_lead_ECGFounder.pth', map_location='cpu')
    model.load_state_dict(ckpt['state_dict']) 
    out = model(x)
