...
def load_pulseppg_from_checkpoint(checkpoint_path, device="cpu"):
    ...
    print("Loaded encoder weights from:", checkpoint_path)
    if missing:
...
if __name__ == "__main__":
    ...
    seq_len = sr * seconds
    batch_size = 2
    x = torch.randn(batch_size, seq_len) # 10s * 50 Hz
    device = "cuda" if torch.cuda.is_available() else "cpu"
    # model = PulsePPG().to(device)
    model = load_pulseppg_from_checkpoint("checkpoints/pulseppg/checkpoint_best.pkl", device=device)
