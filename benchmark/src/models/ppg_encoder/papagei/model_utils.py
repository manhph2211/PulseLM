...
def extract_labels(y, label, binarize_val=None):
    ...
    if label == "optype":
        dict_label = {
            'Colorectal': 0, 'Biliary/Pancreas': 1, 'Stomach': 2, 'Others': 3,
            'Major resection': 4, 'Minor resection': 5, 'Breast': 6,
            'Transplantation': 7, 'Thyroid': 8, 'Hepatic': 9, 'Vascular': 10
        }
...
def load_linear_probe_dataset_objs(dataset_name, model_name, label, func, content, level, string_convert=True, classification=True, concat=True, prefix="../"):
    ...
    X_train, y_train, train_keys = func(df=df_train, dict_embeddings=dict_train, case_name=case_name, label=label, level=level)
    X_val, y_val, val_keys = func(df=df_val, dict_embeddings=dict_val, case_name=case_name, label=label, level=level)
    X_test, y_test, test_keys = func(df=df_test, dict_embeddings=dict_test, case_name=case_name, label=label, level=level)

    if classification:
        y_train = extract_labels(y=y_train, label=label, binarize_val=binarize_val)
        y_val = extract_labels(y=y_val, label=label, binarize_val=binarize_val)
        y_test = extract_labels(y=y_test, label=label, binarize_val=binarize_val)
