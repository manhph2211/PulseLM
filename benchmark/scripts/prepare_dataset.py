...
def _extract_signals(mat: Dict[str, Any]) -> np.ndarray:
    ...
    if sig.ndim == 1:
        sig = sig[None, :]
    if sig.ndim == 2 and sig.shape[0] > 200 and sig.shape[1] < 200:
        sig = sig.T
    if sig.ndim != 2:
...
def build_one_mat(
    ...
    dataset_name, signals, examples = collect_examples_from_mat(
        mat_path=mat_path, default_fs=default_fs, seed=seed,
        signal_ref_path="signals.npy", signal_index_offset=0)
    if not examples:
    ...
    print(f"[OK] {dataset_name}: signals={signals.shape} examples={len(examples)} "
...
def build_merged_dataset(
    ...
    for mat_path in mat_paths:
        dataset_name, signals, examples = collect_examples_from_mat(
            mat_path=mat_path, default_fs=default_fs, seed=seed,
            signal_ref_path="signals.npy", signal_index_offset=signal_offset)
        if not examples:
            print(f"[SKIP] {mat_path} produced 0 examples after filtering")
            continue
        ...
        print(f"[MERGE] added {dataset_name}: signals={signals.shape} examples={len(examples)}")
    ...
    _write_jsonl(os.path.join(out_dir, "dev.jsonl"), dev_rows, "dev")
    _write_jsonl(os.path.join(out_dir, "test.jsonl"), test_rows, "test")
    print(f"[OK] merged dataset: signals={all_signals.shape} examples={len(merged_examples)} "
          f"train/dev/test={len(train_rows)}/{len(dev_rows)}/{len(test_rows)} -> {out_dir}")
...
def main():
    ...
    ap.add_argument("--mode", type=str, default="separate", choices=["separate", "merged", "both"])
    ap.add_argument("--merged_name", type=str, default="merged_ppg_text")
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--default_fs", type=int, default=125)
    ap.add_argument("--split", type=str, default="8,1,1")
    args = ap.parse_args()
    ...
    if args.mode in ("separate", "both"):
        for mat_path in mats:
            try:
                build_one_mat(mat_path=mat_path, out_root=args.out_dir,
                              split_ratio=split_ratio, seed=args.seed, default_fs=args.default_fs)
            except Exception as exc:
                print(f"[FAIL] {mat_path}: {exc!r}")

    if args.mode in ("merged", "both"):
        try:
            build_merged_dataset(mat_paths=mats, out_dir=os.path.join(args.out_dir, args.merged_name),
                                 split_ratio=split_ratio, seed=args.seed, default_fs=args.default_fs)
        except Exception as exc:
